...
if option == "샘플 데이터":
    ...
    text = st.sidebar.text_area("데이터를 CSV 형식으로 입력하세요 (예: x,y,카테고리\n1,10,A\n2,20,B)")
    if text:
        try:
...
if data is not None:
    ...
    if len(columns) >= 2:
        st.subheader("막대 그래프")
        x_col = st.selectbox("X축 선택", columns, key="bar_x_kr")
        y_col = st.selectbox("Y축 선택", columns, key="bar_y_kr")
        if x_col and y_col:
            fig, ax = plt.subplots()
    ...
    if len(columns) >= 2:
        st.subheader("선 그래프")
        x_col = st.selectbox("X축 선택", columns, key="line_x_kr")
        y_col = st.selectbox("Y축 선택", columns, key="line_y_kr")
        if x_col and y_col:
            fig, ax = plt.subplots()
    ...
    if len(columns) >= 2:
        st.subheader("산점도")
        x_col = st.selectbox("X축 선택", columns, key="scatter_x_kr")
        y_col = st.selectbox("Y축 선택", columns, key="scatter_y_kr")
        if x_col and y_col:
    ...
    if len(columns) >= 1:
        st.subheader("히스토그램")
        col = st.selectbox("컬럼 선택", columns, key="hist_kr")
        if col:
            fig, ax = plt.subplots()
            ax.hist(data[col])
            ax.set_xlabel(col)
    ...
    if len(columns) >= 2:
        ...
        cat_col = st.selectbox("카테고리 컬럼 선택", columns, key="cat_bar_cat_kr")
        val_col = st.selectbox("값 컬럼 선택", columns, key="cat_bar_val_kr")
